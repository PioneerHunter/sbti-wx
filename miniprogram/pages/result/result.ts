...
interface TypeInfo {
  code: string; cn: string; intro: string; desc: string;
}
interface NormalType {
  code: string; pattern: string;
}
interface DimLevelMap { L: string; M: string; H: string }

const dimensionMeta: Record<string, { name: string }> = {
  S1: { name: 'S1 自尊自信' }, S2: { name: 'S2 自我清晰度' }, S3: { name: 'S3 核心价值' },
  E1: { name: 'E1 依恋安全感' }, E2: { name: 'E2 情感投入度' }, E3: { name: 'E3 边界与依赖' },
  A1: { name: 'A1 世界观倾向' }, A2: { name: 'A2 规则与灵活度' }, A3: { name: 'A3 人生意义感' },
  Ac1: { name: 'Ac1 动机导向' }, Ac2: { name: 'Ac2 决策风格' }, Ac3: { name: 'Ac3 执行模式' },
  So1: { name: 'So1 社交主动性' }, So2: { name: 'So2 人际边界感' }, So3: { name: 'So3 表达与真实度' },
}

const dimensionOrder = ['S1','S2','S3','E1','E2','E3','A1','A2','A3','Ac1','Ac2','Ac3','So1','So2','So3']

const TYPE_LIBRARY: Record<string, TypeInfo> = {
...
const DIM_EXPLANATIONS: Record<string, DimLevelMap> = {
...
Page({
  onLoad() {
    ...
    const shuffledQuestions = app.globalData.shuffledQuestions ?? []
    ...
    if (drunkTriggered) {
      finalCode = 'DRUNK'
      modeKicker = '隐藏人格已激活'
      matchBadge = '匹配度 100% · 酒精异常因子已接管'
      typeSub = '乙醇亲和性过强，系统已直接跳过常规人格审判。'
      isSpecial = true
    } else if (bestNormal.similarity < 60) {
      finalCode = 'HHHH'
      modeKicker = '系统强制兜底'
      matchBadge = `标准人格库最高匹配仅 ${bestNormal.similarity}%`
      typeSub = '标准人格库对你的脑回路集体罢工了，于是系统把你强制分配给了 HHHH。'
      isSpecial = true
    } else {
      finalCode = bestNormal.code
      modeKicker = '你的主类型'
      matchBadge = `匹配度 ${bestNormal.similarity}% · 精准命中 ${bestNormal.exact}/15 维`
      typeSub = '维度命中度较高，当前结果可视为你的第一人格画像。'
    }

    const typeInfo = TYPE_LIBRARY[finalCode]

    // 十五维度列表
    const dimItems = dimensionOrder.map(dim => ({
      dim,
      name: dimensionMeta[dim].name,
      level: levels[dim],
      score: rawScores[dim],
      explanation: DIM_EXPLANATIONS[dim][levels[dim] as 'L' | 'M' | 'H'],
    }))

    this.setData({
      posterSrc: TYPE_IMAGES[finalCode] ?? '',
      posterCaption: typeInfo.intro,
      modeKicker,
      typeName: `${typeInfo.code}（${typeInfo.cn}）`,
      matchBadge,
      typeSub,
